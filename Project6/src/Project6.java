...
public class Project6 {
	...
	public static void main(String[] args) throws FileNotFoundException
	{
		...
		//Declare and construct a Scanner that is attached to the keyboard of the computer
		Scanner keyboard = new Scanner(System.in);
		
		// Get the hidden word from the file by calling the getWordFromFileMethod
		
		// Call the getWordFromFile method
		String chosenWord = getWordFromFile(NUMBER_WORDS, keyboard);
		
		
		// Convert the word to lower case.
		// Convert the word that is returned from the method call to lower case
		chosenWord = chosenWord.toLowerCase();
		
		// Counting loop
		// Let the user make MAX_GUESSES. Stop if they get all the characters correct
		// Write a loop that lets the user make six guesses as to what the hidden word is
		int guesses = 0;
		int correctPosition = 0;
		while(guesses < MAX_GUESSES && correctPosition != WORD_LENGTH)
		{
			// Let the user enter their guess and convert it to lower case
			System.out.println("Enter your guess. Remember the word has 5 letters");
			String userGuess = keyboard.nextLine();
			userGuess = userGuess.toLowerCase();
			
			// Call the showResult method to show progress to the user
			correctPosition = showResult(chosenWord, userGuess);
			guesses ++;
			System.out.println();
		...
		System.out.println(guesses + " / 6");
		// If they didn't guess the word, tell them what it was.
		// Let the user know what the hidden word was if they didn't guess it
	...
	public static String getWordFromFile(int size, Scanner keyboard) throws FileNotFoundException
	{
		...
		// Let the user tell you which word number they want to play--think about newline characters and whether
		// they need to be read
		System.out.println("Which word number would you like? The maximum is 255");
		int wordNum = keyboard.nextInt();
		keyboard.nextLine();
		// Skip number of lines in the file that the user entered--the first word was at 0, so this is not off by one
		// This should use a loop
		
		
		// Skip over the stuff we don't need in the line of the file that contains the word we want to read
	
		// Read in the word we do need
		...
		String wordOfDay = file.next();
		// Close the file
		file.close();
		
		// Return the word read to end the method
		
		return wordOfDay; // this is a stub
	}
	...
	{
		// step through the user's guess one character at a time--this should be a loop
			int count = 0;
			int index = 0;
			...
			// Determine the fate of the letter--order here is important--this should be a nested if else statement
		
		// Return the number of letters that are in the correct place--this is how you figure out if the user won
